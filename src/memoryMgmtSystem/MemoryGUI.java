...
public class MemoryGUI extends JFrame implements ActionListener, Observer {
	...
	private static final String instructions = "<html><center><font color='red'>Welcome to our Buddy System!</font><br><br>"
			...
			+ "3. <font color='gray'><em> To perform a random animation press 'Random'</em> </font><br>"
	...
	private static JPanel buttonBar = new JPanel(); // Holds all the main user
													// buttons
	...
	public void makeFrame() {
		...
		if (data.startUsed == false) { // Checks if user has already read the
										// instructions
	...
	/*
	 * The Start method provides the user with a basic set of instructions when
	 * they first start to use the program. A 'Start' button is present once the
	 * user is prepared to being using the buddy sysem for memory management.
	 */
	private void Start() {
	...
	private void makeGrid() {
		...
		data.p1.setPreferredSize(new Dimension(data.MEM_BLOCK_WIDTH,
				data.MEM_BLOCK_SCALE));
		data.contentPane.add(data.p1, BorderLayout.WEST);

	}

	public void update(Observable o, Object obj) {
		Border bottomBorder = BorderFactory.createMatteBorder(0, 0, 3, 0,
				Color.BLACK);
		Border topBorder = BorderFactory.createMatteBorder(3, 0, 0, 0,
				Color.BLACK);
		...
		for (int i = 0; i < memory.size(); i++) {
			...
			if (temp.getName() == null) {
				...
				// loop used to color each memory item and set a border to
				// differentiate between different chunks of memory
				for (int j = 0; j <= end - start; j++) {
					if (j != end - start && j != 0) {
						JTextArea co = new JTextArea();
						...
						Color color = Color.BLUE;
						co.setBackground(color);
						co.setBorder(topBorder);
						data.p1.add(co);
					} else {
						JTextArea co = new JTextArea();
						Color color = Color.BLUE;
						co.setBackground(color);
						co.setBorder(bottomBorder);
						data.p1.add(co);
					}
				}
			}
	...
	private void add() {
		...
		// Checks if the name already exists in memory, asks for a new one if it
		// does
		...
		// Checks to see if user entered a positive number for the size of the
		// memory item
		try {
	...
	private void remove() {
		...
		if (name.equals("")) {
			...
			JOptionPane.showMessageDialog(data.frame, name
					+ " not found in memory.", "Name error",
	...
	private void random() {
		...
		String[] names = { "Terminator", "Slicer", "Ninja", "cow", "Robot",
				"littlegirl", "Apple", "Mango", "Peach", "Banana", "Orange",
				"Grapes", "Watermelon", "Tomato", "Joe", "Tom", "Hawiar",
				"Baliga", "OS", "a", "A", "B", "b", "C", "D", "c", "d", "e",
				"f", "g", "h", "E", "F", "G", "H", "bad", "easy", "lol",
				"Hurt", "code", "hate", "kill", "ice", "fire", "icecream",
				"man", "destroy", "computer", "book", "dictionary",
				"technology", "power", "thunder", "controller", "dexterity",
				"keyboard", "thunderous", "blizzard", "hazardous", "algorithm",
				"destruction", "operation", "assignment", "despicable" };
		
		ArrayList<String> population = new ArrayList<String>(); //stores names of items in memory
		
		long current;
		long MAX_RUN_TIME = 300000;
		long startTime = System.currentTimeMillis();
		do {
			boolean choice = rand.nextBoolean();
			if (choice) {
				// pick a random size from 1-32
				int size = numRand.nextInt((32 - 1) + 1) + 1;
				...
				if(population.size() > 1) {
					int index = numRand.nextInt((population.size()-1)+1) + 1;
					String name = population.get(index-1);
		...
		} while(current - startTime < MAX_RUN_TIME);
	}
